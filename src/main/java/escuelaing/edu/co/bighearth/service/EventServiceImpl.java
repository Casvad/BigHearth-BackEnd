...
public class EventServiceImpl implements EventService{
    ...
    private void populateSampleData() {
        Event newEvent1 = new Event( 0 ,20 , "Asistencia a la mujer y la infancia " , "COMUNITARIO",
        ...
        userService.getUser("carlos.ramirez-ot").addEventList(newEvent1);

        newEvent1.getVolunteers().add(userService.getUser("carlos.ramirez-ot").getUsername());
    }
    ...
    @Override
    public Volunteer leaveEventUser(int idEvent, Volunteer volunteer) {
        for (Event event : events){
            if(event.getId() == idEvent){
                return event.removeVolunteer(volunteer);
            }
        }
        return volunteer;
    }
